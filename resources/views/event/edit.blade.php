...
@section('content_body')
    <x-form-layout title="Edit Event: {{ $event->title }}" action="{{ route('event.update', $event->slug) }}" method="POST"
        :hasFiles="true">
        @method('POST')
                ...
                <div class="card shadow-sm">
                    <div class="card-body">
                        ...
                        <div class="form-group mt-4">
                            ...
                            <textarea class="form-control editor @error('description') is-invalid @enderror" name="description" rows="5">{{ old('description', $event->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                            ...
                            @error('feature_image')
                                @if ($event->feature_image)
                                    <div class="current-image mb-3">
                                        ...
                                        <img src="{{ asset('images/' . $event->feature_image) }}" alt="Current Feature Image"
                            ...
                            <select class="form-control select2 @error('galleries_id') is-invalid @enderror"
                                ...
                                @foreach ($galleries as $gallery)
                                    <option value="{{ $gallery->id }}"
                                        {{ old('galleries_id', $event->gallery?->id) == $gallery->id ? 'selected' : '' }}>
